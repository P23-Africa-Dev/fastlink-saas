      ...
      <div className="bg-white rounded-2xl w-full max-w-xl flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-[#f0f0f5] bg-[#f8f8fc]" style={{ padding: "20px 24px" }}>
          <div>
            ...
            <p className="text-[12px] text-[#9ca3af]" style={{ marginTop: "2px" }}>{project.name}</p>
          </div>
        ...
        <div className="overflow-y-auto" style={{ padding: "24px", maxHeight: "70vh", display: "flex", flexDirection: "column", gap: "18px" }}>
          ...
          <div className="flex flex-col" style={{ gap: "8px" }}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} className={inputCls} style={{ padding: "12px 16px" }} />
          </div>
          ...
          <div className="flex flex-col" style={{ gap: "8px" }}>
            ...
            <div className="flex flex-wrap items-center" style={{ gap: "8px" }}>
              {PROJECT_STATUSES.map(s => {
                const cfg = PROJECT_STATUS_CONFIG[s]; const active = status === s;
                return <button key={s} type="button" onClick={() => setStatus(s)} style={{ padding: "7px 16px", borderRadius: "8px", fontSize: "12px", fontWeight: "700", cursor: "pointer", border: `1.5px solid ${active ? cfg.color : "#f0f0f5"}`, background: active ? cfg.bg : "white", color: active ? cfg.color : "#9ca3af", transition: "all 0.15s" }}>{cfg.label}</button>;
              })}
            </div>
          </div>
        ...
        <div className="border-t border-[#f0f0f5] flex items-center justify-end bg-[#f8f8fc]" style={{ padding: "20px 24px", gap: "12px" }}>
          ...
          <ModalButton variant="primary" disabled={!name.trim()} onClick={handleSave}>Save Changes</ModalButton>
        </div>
      </div>
    </div>
  );
