...
export function DeleteTaskModal({ taskTitle, onClose, onConfirm }: DeleteTaskModalProps) {
  return (
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm flex flex-col shadow-2xl overflow-hidden">
        <div className="flex flex-col items-center text-center" style={{ padding: "32px 28px", gap: "16px" }}>
          ...
          <div className="flex flex-col" style={{ gap: "8px" }}>
            <h2 className="text-[18px] font-bold text-(--text-primary)">Delete Task?</h2>
            <p className="text-[13px] text-[#9ca3af] leading-relaxed">
              This will permanently delete <span className="font-bold text-(--text-primary)">{taskTitle}</span> and all its comments. This cannot be undone.
            </p>
          </div>
        </div>
        <div className="border-t border-[#f0f0f5] flex items-center justify-end bg-[#f8f8fc]" style={{ padding: "16px 24px", gap: "12px" }}>
          ...
          <ModalButton variant="danger" onClick={() => { onConfirm(); onClose(); }}>Delete Task</ModalButton>
        </div>
      </div>
    </div>
  );
