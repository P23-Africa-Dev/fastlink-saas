...
import { X, Pencil, Trash2, Check, Plus, GripVertical, Lock } from "lucide-react";
...
function PipelineRowForm({
  ...
  const handlePrivateChange = (isPrivate: boolean) => {
    setForm((f) => ({
      ...
      is_default: isPrivate ? false : f.is_default,
    }));
  };
  ...
  return (
    <div className="rounded-2xl border border-(--accent-purple) bg-[#f8f8fc]" style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        <div className="flex flex-col" style={{ gap: "8px" }}>
          <label className="flex items-center cursor-pointer" style={{ gap: "8px" }}>
            <input
              ...
              disabled={!allowOpenPipelines && form.is_private}
              onChange={(e) => {
                if (!allowOpenPipelines && !e.target.checked) return;
                handlePrivateChange(e.target.checked);
              }}
              className="rounded"
            />
            <span className="text-[12px] font-bold text-(--text-primary)">Private pipeline</span>
          </label>
          {allowDefault && !form.is_private && (
            <label className="flex items-center cursor-pointer" style={{ gap: "8px" }}>
              <input
                type="checkbox"
      ...
      {!allowOpenPipelines && (
        <p className="text-[11px] text-[#9ca3af]">Your role can only create private pipelines.</p>
      )}

      <div className="flex items-center justify-end" style={{ gap: "8px" }}>
        <ModalButton variant="secondary" onClick={onCancel} style={{ padding: "6px 14px" }}>
          Cancel
        </ModalButton>
        <ModalButton
          variant="primary"
...
export function ManagePipelinesModal({
  ...
  const EMPTY_FORM: RowFormState = {
    ...
    is_default: false,
    is_private: defaultPrivate || !allowOpenPipelines,
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg flex flex-col shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between border-b border-[#f0f0f5] bg-[#f8f8fc]" style={{ padding: "20px 24px" }}>
          <div>
            ...
            <p className="text-[12px] text-[#9ca3af]" style={{ marginTop: "2px" }}>
              Create, edit, and set privacy on your sales pipelines.
            </p>
          </div>
        ...
        <div className="overflow-y-auto flex flex-col" style={{ padding: "20px 24px", maxHeight: "60vh", gap: "8px" }}>
              ...
              {editingId === drive.id ? (
                <PipelineRowForm
                  initial={{
                    ...
                    color: drive.color,
                    is_default: drive.is_default,
                    is_private: Boolean(drive.is_private),
                  }}
                  allowOpenPipelines={allowOpenPipelines || !drive.is_private}
                  onSave={(data) => {
                    onUpdate(drive.id, data);
                    setEditingId(null);
                  }}
                ...
                <div className="rounded-2xl border border-red-200 bg-red-50 flex items-center justify-between" style={{ padding: "14px 16px" }}>
                  <p className="text-[13px] font-bold text-red-700">
                    Delete <span className="italic">{drive.name}</span>?
                  </p>
                  <div className="flex items-center" style={{ gap: "8px" }}>
                    <button
                      onClick={() => setConfirmDeleteId(null)}
                      className="text-[12px] font-bold text-[#9ca3af] hover:text-(--text-primary) transition-colors"
                    >
                      Cancel
                    </button>
                    <ModalButton
                ...
                <div
                  className="flex items-center rounded-2xl border border-[#f0f0f5] bg-white hover:border-[#e5e7eb] transition-all group"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center" style={{ gap: "6px" }}>
                      ...
                      {drive.is_default && (
                        <span className="rounded-full text-[10px] font-bold" style={{ padding: "1px 7px", background: "#33084E15", color: "#33084E" }}>
                          Default
                        </span>
                      )}
                      {drive.is_private && (
                        <span
                          className="rounded-full text-[10px] font-bold inline-flex items-center"
                          style={{ padding: "1px 7px", gap: "3px", background: "#AF580B15", color: "#AF580B" }}
                        >
                    ...
                    {drive.description && (
                      <p className="text-[11px] text-[#9ca3af] truncate" style={{ marginTop: "2px" }}>
                        {drive.description}
                      </p>
                    )}
                  </div>
                  {(drive.can_edit !== false || drive.can_delete !== false) && (
                    <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity" style={{ gap: "4px" }}>
                      {drive.can_edit !== false && (
                        <button
          ...
          {addingNew ? (
            <PipelineRowForm
              ...
              onSave={(data) => {
                onCreate({ ...data, position: drives.length + 1 });
                setAddingNew(false);
              }}
